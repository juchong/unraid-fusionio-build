...
if [ "${CUSTOM_MODE}" == "true" ]; then
	echo "---Everything prepared, exiting---"
	#exit 0
fi
...
if [ "${BUILD_IOMEMORY}" == "true" ]; then
	echo "---Installing tools needed for compiling the FusionIO/ioMemory Driver---"
	apt update
	apt install -y dkms
	echo "---Building the ioMemory Driver for 5.x Kernels---"
	rm -r ${DATA_DIR}/iomemory-vs* 2> /dev/null
	rm -r /var/lib/dkms/iomemory-vs* 2> /dev/null
	cd ${DATA_DIR}
	git clone https://github.com/juchong/iomemory-vsl4.git
	cd ${DATA_DIR}/iomemory-vsl4/
...
## Modify the udev rules for ioMemory
#sed -i 's/|nvme\*|/&fio\*|/' ${DATA_DIR}/bzroot-extracted-$UNAME/lib/udev/rules.d/60-persistent-storage.rules
#touch ${DATA_DIR}/bzroot-extracted-$UNAME/lib/udev/rules.d/99-fio-link.rules
#echo "KERNEL=="fio*", SUBSYSTEMS=="block", SYMLINK+="nvme%nn%n"" >> ${DATA_DIR}/bzroot-extracted-$UNAME/lib/udev/rules.d/99-fio-link.rules
